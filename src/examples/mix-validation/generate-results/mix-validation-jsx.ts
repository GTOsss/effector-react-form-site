...
import { useForm, createForm, createNameHelper } from 'effector-react-form';
...
const formValidate = ({ values, errorsInline }) => {
  const errors = {};
  const nameHelper = createNameHelper();
  const password = nameHelper.getStr('password');
  const passwordRepeat = nameHelper.getStr('passwordRepeat'); // errorsInline contain fields-level errors
